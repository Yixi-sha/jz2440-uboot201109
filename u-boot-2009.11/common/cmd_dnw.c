...
int do_dnw(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    int i;
    size_t len = ~0UL;
    char buf[32];
    ...
    /* 
     * download_run为1时表示将文件保存到
     * USB Host发送工具dnw指定的位置.
     * download_run为0时表示将文件保存到
     * 参数argv[2]指定的位置.
     */
    download_run = 1;
    switch (argc) {
...
U_BOOT_CMD(
    dnw,    3,    0,    do_dnw,
    "dnw - get file from host(PC)\n",
    "[wait] [loadAddress]\n"
    "\"wait\" is 0 or 1, 0 means for return immediately, "
    "not waits for the finish of transferring\n"
);
#endif
